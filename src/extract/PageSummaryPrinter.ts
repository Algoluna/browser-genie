...
import { PageSummary, StructuredTextNode } from '../types';
import { marked } from 'marked';
import TerminalRenderer from 'marked-terminal';

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

export class PageSummaryPrinter {
  constructor(private readonly model: string = 'gpt-4o') {}

  async format(summary: PageSummary): Promise<string> {
    const cleanedSummary = {
      title: summary.title,
      url: summary.url,
      visibleText: this.normalizeStructuredTextTree(summary.visibleText),
    };

    const compactJSON = JSON.stringify(cleanedSummary);
...
`.trim();
  ...
  async printToTerminal(markdown: string) {
    marked.setOptions({
  ...
  private normalizeStructuredTextTree(
    ...
    if (depth >= maxDepth) {
      const flatText: StructuredTextNode[] = this.flattenText(node);
      return {
        tag: 'div',
        children: flatText
      };
    ...
    const cleanedChildren: StructuredTextNode[] = [];
  ...
  private flattenText(node: StructuredTextNode): StructuredTextNode[] {
    if (node.tag === 'text') return [node];
