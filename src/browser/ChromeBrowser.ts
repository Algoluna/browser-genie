...
import { AnnotatedElement, ExecutionContext, PageState, PageSummary, StructuredTextNode } from '../types';
...
function getBoxBoundsFromQuad(quad: number[]): [number, number, number, number] {
...
async function getBoundingBox(client: any, objectId: string): Promise<[number, number, number, number]> {
...
export class ChromeBrowser implements Browser {
  ...
  async launch(url: string): Promise<ExecutionContext> {
    ...
    const executionContext = new ExecutionContext(targetId);

    return executionContext;
  }

  async captureState(context: ExecutionContext): Promise<PageState> {
    const tab = await CDP({ target: context.targetId });
  ...
  async click(context: ExecutionContext, objectId: string): Promise<void> {
  ...
  async hover(context: ExecutionContext, objectId: string): Promise<void> {
    const { Input } = context.client;
    const [x1, y1, x2, y2] = await getBoundingBox(context.client, objectId);
    const x = (x1 + x2) / 2;
    const y = (y1 + y2) / 2;
  ...
  async type(context: ExecutionContext, objectId: string, text: string): Promise<void> {
  ...
  async pressEscape(context: ExecutionContext): Promise<void> {
    const { Input } = context.client;
  ...
  async goback(context: ExecutionContext): Promise<void> {
    const { Page } = context.client;
    await Page.goBack();
    await waitForDocumentReady(context.client);
  }
  
  async extractSummary(context: ExecutionContext): Promise<PageSummary> {
  ...
  Array.from(document.images)
    ...
    return new PageSummary(
      titleResult.value,
      urlResult.value,
      textResult.value as StructuredTextNode,
      imageResult.value as { src: string; alt: string }[]
    );
  }  
