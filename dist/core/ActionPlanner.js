...
class ActionPlanner {
    ...
    async generateActionPlan(instruction, elements, screenshotPath, executionContext) {
    ...
    async expandUserInstruction(instruction, screenshotPath, executionContext) {
        const screenshotData = fs_1.default.readFileSync(screenshotPath).toString('base64');
...
`;
    ...
    async resolveReferences(elements, instruction) {
        const resolvedMap = {};
...
Only return valid JSON. No commentary.`.trim()
            ...
            for (const el of batch) {
                ...
                if (el.clipPath && fs_1.default.existsSync(el.clipPath)) {
                    const imageData = fs_1.default.readFileSync(el.clipPath).toString('base64');
                    content.push({
                        type: 'image_url',
            ...
            try {
                ...
                for (const entry of json.resolvedReferences || []) {
                    const list = (entry.objectIds || []).sort((a, b) => b.confidence - a.confidence);
                    if (list.length)
                        resolvedMap[entry.reference] = list;
                }
            }
            catch (err) {
    ...
    async planActions(expandedInstruction, resolvedMap, screenshotPath) {
        const screenshotData = fs_1.default.readFileSync(screenshotPath).toString('base64');
        const promptText = `
...
`.trim();
        ...
        if (!result)
            throw new Error('No action plan returned by OpenAI');
        try {
